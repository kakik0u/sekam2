...
async def setup_admin_commands(tree, client):
    ...
    """
    @tree.command(name="overload", description="過負荷モードのON/OFFを切り替えます（管理者専用）")
    ...
    async def overload(ctx: discord.Interaction, enable: Optional[bool] = None):
        ...
        try:
            ...
            if int(getattr(ctx.user, "id", 0) or 0) != 668479297551466516:
                await ctx.response.send_message("テメーの頭が高負荷だろ。", ephemeral=True)
            ...
            await ctx.response.send_message(f"過負荷モードを {state} にしました。", ephemeral=True)
    ...
    async def sync(ctx: discord.Interaction):
        try:
            if await enforce_zichi_block(ctx, "/sync"):
                return
            if int(getattr(ctx.user, "id", 0) or 0) != 668479297551466516:
                await ctx.response.send_message("使用禁止っつったよな？", ephemeral=True)
            ...
            await handle_command_error(ctx, "/sync", e, "コマンド同期中にエラーが発生しました。")
