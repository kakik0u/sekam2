...
class GraphPaginationView(discord.ui.View):
    ...
    def __init__(self, all_data: list, username: str, reference_label: str, graph_type: str, user_id: int):
        """
        Args:
    ...
    def update_buttons(self):
        ...
        self.children[3].style = discord.ButtonStyle.success if self.show_others else discord.ButtonStyle.secondary
    ...
    async def update_graph(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # メッセージを更新
            file = discord.File(image_path, filename=f"{self.graph_type}_distribution.png")
            ...
            if debug:
                ...
                traceback.print_exc()
            try:
                await interaction.followup.send("グラフの更新中にエラーが発生しました。", ephemeral=True)
            except Exception:
                pass
    ...
    @discord.ui.button(label="リセット", style=discord.ButtonStyle.secondary, emoji="🔄")
    ...
    @discord.ui.button(label="その他: オン", style=discord.ButtonStyle.success, emoji="📊")
    async def toggle_others_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        """その他の表示をオンオフ"""
...
def create_channel_graph(data: list, username: str, reference_label: str, status_text: str = "") -> str:
    """
    ...
    try:
        ...
        # 背景画像と合成
        bg_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "bg/bg_green.png")
        if os.path.exists(bg_path):
            ...
            max_graph_height = int(bg_height * 0.70)  # 背景の70%まで（下部に文字用余白確保）
            
            if graph_width > max_graph_width or graph_height > max_graph_height:
                # アスペクト比を維持してリサイズ
                ratio = min(max_graph_width / graph_width, max_graph_height / graph_height)
            ...
            with Pilmoji(final_img) as pilmoji:
                ...
                title_x = 50  # 左端から50px
                title_y = 40
                pilmoji.text((title_x, title_y), title_text, font=title_font, fill='white')
                
                # 参照ラベルを左下に描画
                clean_label = reference_label.replace('-# ', '').replace('-#', '')
                label_x = 50  # 左端から50px
                label_y = bg_height - 60  # 下から60px
                pilmoji.text((label_x, label_y), clean_label, font=label_font, fill='white')
                
                # 状態テキストを表示（参照ラベルの一行上）
                if status_text:
                    status_x = 50
                    status_y = label_y - 30  # 参照ラベルの30px上
                    pilmoji.text((status_x, status_y), status_text, font=label_font, fill='#AAAAAA')
                
                # X軸のラベルを描画（チャンネル名）
                # グラフの目盛り・余白を除外してバー表示領域のみで計算
                graph_left_margin = 80  # グラフ左側の目盛り余白（固定）
                graph_right_margin = 20  # グラフ右側の余白（固定）
                usable_width = graph_width - graph_left_margin - graph_right_margin  # 実際のバー表示領域
                bar_width = usable_width / len(channel_labels)  # 各バーの幅
                
                # グラフの下部にラベルを配置（余白を増やす）
                label_y_pos = y_offset + graph_height + 15  # グラフの下15px（10px→15px）
                
                for i, label in enumerate(channel_labels):
                    # 各ラベルの位置を計算（左から右へ、目盛り余白を考慮）
                    label_x_pos = x_offset + graph_left_margin + int(i * bar_width + bar_width / 2)
                    ...
                    pilmoji.text((label_x_pos, label_y_pos), display_label, font=channel_label_font, fill='white')
...
def create_reaction_graph(data: list, username: str, reference_label: str, status_text: str = "") -> str:
    """
    ...
    try:
        ...
        # 背景画像と合成
        bg_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), "bg/bg_blue.png")
        if os.path.exists(bg_path):
            # 背景画像を読み込み
            bg = Image.open(bg_path).convert('RGBA')
            graph_img = Image.open(graph_temp_path).convert('RGBA')
            ...
            max_graph_height = int(bg_height * 0.75)  # 背景の75%まで（70%→75%に拡大、さらに36px削る）
            ...
            with Pilmoji(final_img) as pilmoji:
                ...
                if status_text:
                    ...
                    pilmoji.text((status_x, status_y), status_text, font=status_font, fill='#AAAAAA')
                ...
                for i, label in enumerate(emoji_labels):
                    # 各ラベルの位置を計算（左から右へ）
                    label_x_pos = x_offset + int(left_margin + i * bar_width + bar_width / 2 - 16)
                    
                    # "その他"の場合は通常フォント、それ以外は絵文字として扱う
                    if label == "その他":
                        # 通常フォントで描画
                        pilmoji.text((label_x_pos, label_y_pos), label, font=label_font, fill='white')
                    else:
                        # 絵文字を描画（Pilmojiが自動的にカラー絵文字として描画）
                        try:
                            pilmoji.text((label_x_pos, label_y_pos), label, font=emoji_label_font, 
                                        fill='white', emoji_scale_factor=1.2)
                        except Exception as e:
                            if debug:
                                print(f"Pilmoji絵文字描画エラー ({label}): {e}")
                            # フォールバック: 英名を表示
                            original_name = data[i][0]
                            pilmoji.text((label_x_pos, label_y_pos), original_name, font=label_font, fill='white')
...
async def setup_graph_commands(tree, client):
    ...
    @tree.command(name="myreaction", description="あなたが受け取ったリアクションの分布を表示します")
    ...
    async def myreaction(ctx: discord.Interaction):
        ...
        try:
            ...
            if not rows or len(rows) == 0:
                ...
                await ctx.followup.send(f"{username}のもらったリアクション分布\n{reference_label}", embed=embed)
            ...
            view = GraphPaginationView(all_data, username, reference_label, 'reaction', uid)
            ...
            # 初期グラフを生成
            image_path = create_reaction_graph(current_data, username, reference_label, status_text)
    ...
    @tree.command(name="mylocate", description="あなたが普段書き込んでいるチャンネル・スレッドの分布を表示します")
    @allowed_installs(guilds=True, users=True)
    async def mylocate(ctx: discord.Interaction):
        ...
        try:
            ...
            if not rows or len(rows) == 0:
                ...
                embed.set_footer(text="SEKAM2 - SEKAMの2", icon_url="https://d.kakikou.app/sekam2logo.png")
                await ctx.followup.send(f"{username}の書き込み先チャンネル分布\n{reference_label}", embed=embed)
            ...
            # GraphPaginationViewを使用して初期グラフを作成
            view = GraphPaginationView(all_data, username, reference_label, 'channel', uid)
            ...
            # 初期グラフを生成
            image_path = create_channel_graph(current_data, username, reference_label, status_text)
