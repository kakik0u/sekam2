...
async def setup_misc_commands(tree, client):
    ...
    async def zichi(ctx: discord.Interaction):
        ...
        try:
            if ctx.user.id == 512942239153127425:
                await ctx.response.send_message("<@668479297551466516> 品田さんがお呼びだぞ")
                insert_command_log(ctx, "/zichi", "SHINADA")
                return
            
            channel = getattr(ctx, 'channel', None)
            if not isinstance(channel, discord.Thread):
                await ctx.response.send_message("おめーがチャンネルを自治できるとでも思ってんのか？？？")
                insert_command_log(ctx, "/zichi", "CHANNEL")
                return
            
            guild = getattr(ctx, 'guild', None)
            if not guild or int(getattr(guild, 'id', 0) or 0) != 518371205452005387:
                await ctx.response.send_message("鯖主に言って自治してもらいなさい", ephemeral=True)
            ...
            class ZichiReasonModal(discord.ui.Modal, title="自治申請"):
                ...
                async def on_submit(self, interaction: discord.Interaction):
                    ...
                    ok = insert_zichi_request(channel.id, interaction.user.id, r or "(空)")
            ...
            if not ctx.response.is_done():
                await ctx.response.send_message("エラーが発生しました。", ephemeral=True)
            else:
    ...
    async def wick(ctx: discord.Interaction, message: str):
        ...
        if "菊池真" in message or "2W African Drumming IMG 0001" in message:
            await ctx.response.send_message("こんなこと言えるわけ無いだろ", ephemeral=True)
