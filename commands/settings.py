...
async def setup_settings_commands(tree, client):
    ...
    class setting(app_commands.Group):
        ...
        @app_commands.describe(channel="ログを送信するチャンネル(Botが見えている必要があります。)")
        async def logchsetting(self, ctx: discord.Interaction, channel: discord.TextChannel):
            ...
            if ctx.guild is None:
                await ctx.response.send_message("このコマンドはサーバー内でのみ実行できます。", ephemeral=True)
                insert_command_log(ctx, "/setting log", "DENY_NO_GUILD")
                return
            if not ctx.is_guild_integration():
                await ctx.response.send_message("このコマンドはサーバーに導入されたときのみ使えますよ。", ephemeral=True)
        ...
        async def bansetting(self, ctx: discord.Interaction, setting: str):
            ...
            choice = str(setting)
            ok = set_setting_value(ctx.guild.id, "ban", True if choice == "on" else False)
        ...
        @app_commands.command(name="blacklist", description="ブラックリストに参加するかを設定できます。")
        @guild_only
        @allowed_contexts(guilds=True, dms=False, private_channels=False)
        @allowed_installs(guilds=True, users=False)
        ...
        async def blacklist(self, ctx: discord.Interaction, setting: str):
            ...
            choice = str(setting)
            ok = set_setting_value(ctx.guild.id, "blacklist", True if choice == "on" else False)
            msg = "設定されました。" if ok else "設定に失敗しました。"
